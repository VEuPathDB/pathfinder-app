...
import { getGeneSetVdiPublication, publishGeneSetToVdi } from "../api/geneSets";
import { useInvalidateGeneSets } from "../hooks/useInvalidateGeneSets";
...
function PublishedLine({ geneSetId }: { geneSetId: string }) {
  const status = useQuery({
    queryKey: ["gene-sets", "vdi-publication", geneSetId] as const,
...
export function PublishToVdiButton({ geneSet }: { geneSet: GeneSet }) {
  ...
  return (
    <span className="flex items-center gap-1.5">
      <label className="text-[10px] text-muted-foreground" htmlFor="vdi-visibility">
        Visibility
      </label>
      <select
        id="vdi-visibility"
