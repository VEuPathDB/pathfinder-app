...
describe("seedDemoData", () => {
  ...
  it("parses typed SSE frames and calls onMessage for each event's message", async () => {
    ...
    await seedDemoData((msg) => messages.push(msg));
  ...
  it("throws when response body is null", async () => {
    ...
    await expect(seedDemoData(() => {})).rejects.toThrow(/no response body/);
  });
  ...
  it("forwards siteId as a query parameter when provided", async () => {
    ...
    await seedDemoData(() => {}, "plasmodb");

    expect(fetchSpy).toHaveBeenCalledWith(
      "http://localhost:8000/api/v1/seed?siteId=plasmodb",
