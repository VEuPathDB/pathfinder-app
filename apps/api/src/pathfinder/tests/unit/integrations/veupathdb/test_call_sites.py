...
def test_discovery_does_not_import_services() -> None:
    """The integration layer must not reach up into services."""
    assert (
        _modules_containing("pathfinder.services", under=f"{_INTEGRATION}/discovery.py")
        == []
    )
...
def test_semantic_index_lives_in_integrations() -> None:
    ...
    assert Path(spec.origin).parts[-3:] == (
        "integrations",
        "embeddings",
        "semantic_index.py",
    )
