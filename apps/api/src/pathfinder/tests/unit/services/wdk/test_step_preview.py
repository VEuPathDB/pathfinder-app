...
from pathfinder.services.wdk.step_preview import step_sample_records
...
async def test_a_refused_id_only_read_reaches_the_caller(
    ...
    class _Refusing(_FakeStrategyAPI):
        async def get_step_answer(
            ...
            raise AppError(ErrorCode.WDK_ERROR, "step 5 is not part of a strategy")

    _bind(monkeypatch, _Refusing())

    with pytest.raises(AppError, match="not part of a strategy"):
