...
from pathfinder.services.gene_lookup import MAX_GENE_IDS, normalize_gene_ids
...
async def test_search_listings_carry_the_names_the_catalog_lists(
    ...
    monkeypatch.setattr(catalog, "list_searches", rows)
...
async def test_transform_listings_carry_the_description_the_catalog_lists(
    ...
    monkeypatch.setattr(catalog, "list_transforms", rows)
...
async def test_search_categories_carry_the_counts_the_ontology_groups(
    ...
    monkeypatch.setattr(catalog, "browse_search_categories", rows)
