...
from pathfinder.services.workbench import gene_sets
...
async def test_listing_for_a_user_asks_the_store_for_that_user(
    ...
    found = await gene_sets.list_gene_sets(site_id="plasmodb", user_id=user_id)
...
async def test_listing_without_a_user_asks_the_store_for_the_whole_site(
    ...
    found = await gene_sets.list_gene_sets(site_id="toxodb", user_id=None)
