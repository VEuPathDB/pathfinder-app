...
from pathfinder.platform.errors import AppError, ErrorCode, ValidationError, WDKError
...
def fake_wdk(monkeypatch: pytest.MonkeyPatch) -> _FakeWDK:
    ...
    monkeypatch.setattr(
        pv, "get_discovery_service", lambda: _FakeDiscovery(_definition())
    )
    monkeypatch.setattr(pv, "get_refreshed_dependent_params", no_dependent_refresh)
    return client
...
def static_wdk(monkeypatch: pytest.MonkeyPatch) -> None:
    async def _static(
        ...
        *,
        ...
        return pv.ResolvedSearch(response=_definition(), values_were_read=False)

    monkeypatch.setattr(pv, "_resolve_search_details", _static)
    monkeypatch.setattr(pv, "get_refreshed_dependent_params", no_dependent_refresh)
...
class _FailingContextualClient:
    ...
    async def get_search_details_with_params(
        ...
        *,
        ...
        del record_type, search_name, context, expand_params
        raise AppError(ErrorCode.WDK_ERROR, _WDK_500, status=500)
