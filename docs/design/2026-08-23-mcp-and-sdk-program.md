...
- In-process tools already emit typed parts by returning `ToolReturn(return_value=..., metadata=[DataChunk(...)])` - see `apps/api/src/pathfinder/ai/tools/standalone/workbench.py:87-106` and the chunk builders in `apps/api/src/pathfinder/ai/tools/standalone/stream_parts.py`.
