...
* **Every user-driven UAT flow runs on the mock stack.** The scripted model
  routes on one token a spec writes into its message, `[[arc:<name>]]`, with an
  optional `[[fault:<name>]]`, from a registry of 46 arcs
  (`ai/models/mock/registry.py`); the substring markers are gone, a mistyped
  token fails instead of echoing, and only two product texts route without a
  token (the `/clear` sentence and the attachment marker). Every value an arc
  states comes from the site the test opened (`site_values.py` over the seed
  catalog; the ids, step ids and set ids from the arc's own tool returns through
  `reads.py`), so the suite runs on plasmodb and vectorbase from one `E2E_SITES`
  list, and a contract test runs the real `reconcile()` over every arc's final
  reply. The Lead reads the two scripted context variables through
  `bind_scripted_scope`, inert outside the mock. `apps/web/e2e/uat/` holds one
  spec per flow in eleven family files; the coverage matrix in
  `docs/knowledge/uat/index.md` names the spec, the model check and the fault
  test of every flow. The last full run on the mock stack passed 296 of 300 on both sites; the four remaining flows were one arc's, fixed and re-run green, and the sharded CI run on the pushed tree is the whole-suite record.

* **The model-driven flows are a pre-release report, not a gate.** 58
  `uat-<flow>-<site>` cases join `evals/corpus/` with the effort they were
  measured at, per-gate answers (`GateAnswer`: accept, comment, option picks),
  attachments, a fresh conversation at a named turn, and a recorded root count
  with the site build it held on. `evals/drift.py` judges a run `pass`,
  `re-measure` or `fail` by the exit criteria's tolerance; `evals run` always
  drives the configured provider through the worker. The checks run from a
  machine that already holds the key, never from a public-repository secret, and
  a person reads the verdicts: a real model's answer is a trend, and the
  convention already said an eval gates nothing until it has caught a
  regression and held stable.

* **A wrong model, and the suite in shards.** Thirteen faults
  (`ai/models/mock/fault_calls.py`), each one guard's wrong call once and then
  the arc's own correct call, prove from the browser the refusals and
  corrections the a16 findings named: the qualifier, the repeated control test,
  the transcript unit, the sampled-gene wording, the misnamed deletion, the
  sweep without controls, the rationale refusals, the short card reply, the
  enum, term and vocabulary refusals, an unbacked control count, and a new
  turn-contract rule, `misstated_count`, that refuses a strategy or step count
  the strategy did not hold at the start or the end of the turn. Each guard was
...
* **What the suite found in the product.** Seven findings, FND-15 to FND-22 in
  ...
  browser's error and then nothing; the settings reset missed the store's dated
  key; the file chooser kept the previous model's types (assistant-ui's
  composer snapshot); a saved strategy listed no count; the EDA gene-id list was
  cut at the plot's 4,000 points; another user's conversation address spun on
  the api's 403; and a strategy write was overtaken by an older read in
  flight. Two gaps the flows expose are closed: the Lead renames a strategy
  (`rename_strategy`, the same service the slash command uses), and the live
  state carries each step's WDK id and the root marker so a sweep can name its
  step. A failed verdict names the build's reason whichever side found the
  failure.
