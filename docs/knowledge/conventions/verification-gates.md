...
Two limits are worth knowing. The guard runs per test, so anything at collection time is outside it, including the PIGuard model download the root `conftest.py` performs at import. And a C extension that opens its own socket without going through the `socket` module is not covered.
